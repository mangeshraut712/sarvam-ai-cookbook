...
def get_explanation(
    question: str, language_code: str, grade_level: int, api_key: str
) -> Dict[str, Any]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Homework Helper - Get detailed explanations for student questions"
    )
    parser.add_argument("question", help="The homework question or topic to explain")
    parser.add_argument(
        "--grade", type=int, default=5, help="Student's grade level (default: 5)"
    )
    parser.add_argument(
        "--api-key",
        help="Sarvam API key (can also be set via SARVAM_API_KEY environment variable)",
    )
    ...
    if not api_key:
        raise ValueError(
            "API key must be provided either via --api-key or SARVAM_API_KEY environment variable"
        )
