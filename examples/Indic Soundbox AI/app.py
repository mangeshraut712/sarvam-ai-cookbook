...
def tts_route():
    ...
    print(
        f"/tts route: Received request to synthesize text for TTS: '{text_to_synthesize}' (length: {len(text_to_synthesize)}) in lang: {lang_code}"
    )
    ...
    try:
        ...
        print(
            f"/tts route: Received base64 audio content from TTS module, length: {len(audio_base64_content)}"
        )
...
def get_audio_route(audio_id):
    ...
    if audio_data_uri:
        try:
            ...
            print(
                f"/get_audio route: Found audio for {audio_id}. Decoded bytes length: {len(audio_bytes)}. Mimetype: {mimetype}"
            )
            return Response(audio_bytes, mimetype=mimetype)
        except Exception as e:
            print(
                f"/get_audio route: Error processing stored audio data for {audio_id}: {e}"
            )
