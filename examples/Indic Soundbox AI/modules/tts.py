...
MIN_TEXT_LENGTH_FOR_FORCED_TWO_WAY_SPLIT = (
    20  # Chars, don't split very short texts forcibly
)
...
def _clean_text_for_tts(text_input):
    ...
    print(
        f"[modules/tts.py] _clean_text_for_tts: Original (len {len(text_input)}): '{text_input[:100]}...', Cleaned (len {len(cleaned_text)}): '{cleaned_text[:100]}...'"
    )
    return cleaned_text
...
def _call_sarvam_tts(text_chunk, lang_code, speaker="shubh", model="bulbul:v3"):
    ...
    print(
        f"[modules/tts.py] _call_sarvam_tts: Calling API for chunk (length: {len(text_chunk)}): '{text_chunk[:100]}...'"
    )
    ...
    if (
        not base64_audio_for_chunk or len(base64_audio_for_chunk) < 100
    ...
    print(
        f"[modules/tts.py] _call_sarvam_tts: Successfully received audio for chunk. Base64 length: {len(base64_audio_for_chunk)}"
    )
...
def _chunk_text_boundary_aware(text, max_length):
    ...
    while current_pos < text_len:
        ...
        for delim in sentence_delimiters:
            ...
            if last_occurrence != -1:
                ...
                if (
                    potential_split > found_sentence_split
                ):  # Take the latest occurring delimiter
    ...
    print(
        f"[modules/tts.py] _chunk_text_boundary_aware: Text (len {text_len}) chunked into {len(final_chunks)} pieces."
    )
    return final_chunks
...
def _concatenate_wav_from_base64_list(base64_audio_list):
    ...
    if not base64_audio_list:
        print(
            "[modules/tts.py] _concatenate_wav_from_base64_list: No audio list provided."
        )
        return None
    if len(base64_audio_list) == 1:
        print(
            "[modules/tts.py] _concatenate_wav_from_base64_list: Only one chunk, no concatenation needed."
        )
        return base64_audio_list[0]

    print(
        f"[modules/tts.py] _concatenate_wav_from_base64_list: Concatenating {len(base64_audio_list)} audio chunks."
    )
    ...
    for i, b64_string in enumerate(base64_audio_list):
        try:
            ...
            with io.BytesIO(wav_bytes) as wav_file_in_memory:
                with wave.open(wav_file_in_memory, "rb") as wf:
                    if i == 0:
                        ...
                        print(
                            f"[modules/tts.py] _concatenate_wav_from_base64_list: Audio params from first chunk: {wav_params}"
                        )
                    # Basic check: Sarvam should be consistent, but good to be aware
                    elif (
                        wav_params[:3] != wf.getparams()[:3]
                    ):  # nchannels, sampwidth, framerate
                        error_msg = "Mismatch in critical audio parameters (channels, sampwidth, framerate) between chunks."
                        print(
                            f"[modules/tts.py] _concatenate_wav_from_base64_list: {error_msg}"
                        )
            ...
            print(
                f"[modules/tts.py] _concatenate_wav_from_base64_list: Error processing base64 chunk {i}: {e}"
            )
            raise  # Re-raise, as partial concatenation is problematic

    if not all_frames_data or not wav_params:
        print(
            "[modules/tts.py] _concatenate_wav_from_base64_list: No valid audio frames to concatenate."
        )
        return None
    ...
    print(
        f"[modules/tts.py] _concatenate_wav_from_base64_list: Concatenation successful. Final base64 length: {len(final_concatenated_base64)}"
    )
...
def text_to_speech(text, lang_code, speaker="shubh", model="bulbul:v3"):
    ...
    """
    print(
        f"[modules/tts.py] text_to_speech function called with raw text (length: {len(text)}): '{text[:100]}...', lang_code: {lang_code}, speaker: {speaker}, model: {model}"
    )
    ...
    if not cleaned_text or not cleaned_text.strip():  # Check cleaned_text now
        print(
            "[modules/tts.py] text_to_speech: Empty or whitespace-only text after cleaning."
        )
        silent_wav_base64 = (
            "UklGRkoAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQYAAAAAAQ=="
        )
        print(
            "[modules/tts.py] text_to_speech: Returning placeholder for silent audio due to empty input post-cleaning."
        )
        return silent_wav_base64

    if len(cleaned_text) > TTS_CHARACTER_LIMIT:
        print(
            f"[modules/tts.py] text_to_speech: Cleaned text exceeds limit ({len(cleaned_text)} > {TTS_CHARACTER_LIMIT}). Using boundary-aware chunking."
        )
        ...
        print(
            f"[modules/tts.py] text_to_speech: Cleaned text is very short (len {len(cleaned_text)} < {MIN_TEXT_LENGTH_FOR_FORCED_TWO_WAY_SPLIT}). Treating as a single chunk."
        )
        text_chunks = [cleaned_text.strip()]
    else:
        print(
            f"[modules/tts.py] text_to_speech: Cleaned text (len {len(cleaned_text)}) will be forcibly split into two chunks."
        )
        ...
        split_pos_backward = stripped_text.rfind(
            " ", 0, mid_point + 1
        )  # Include midpoint in search
        ...
        text_chunks = []
        if chunk1:
            text_chunks.append(chunk1)
        if chunk2:
            text_chunks.append(chunk2)

        if (
            not text_chunks
        ):  # Should not happen if original text was not empty and stripped
        ...
        print(
            f"[modules/tts.py] text_to_speech: Forced split resulted in {len(text_chunks)} chunks: {chunk_previews}"
        )

    # Common processing for all chunks
    if not text_chunks:
        print(
            "[modules/tts.py] text_to_speech: No text chunks to process after splitting logic (this indicates an issue)."
        )
    ...
    for i, chunk in enumerate(text_chunks):
        if not chunk:
            print(
                f"[modules/tts.py] text_to_speech: Skipping empty chunk {i+1} in processing loop."
            )
            continue
        print(
            f"[modules/tts.py] text_to_speech: Processing chunk {i+1}/{len(text_chunks)}."
        )
        try:
            ...
            print(
                f"[modules/tts.py] text_to_speech: CRITICAL - Error synthesizing audio for chunk {i+1} ('{chunk[:50]}...'). Error: {e}"
            )
            raise Exception(
                f"Failed to synthesize complete audio due to error in chunk {i+1}. {e}"
            )

    if not base64_audio_parts:
        print(
            "[modules/tts.py] text_to_speech: No audio parts were successfully synthesized after chunking."
        )
        raise Exception("No audio parts were generated from text chunks.")

    print(
        f"[modules/tts.py] text_to_speech: All {len(base64_audio_parts)} chunks processed. Concatenating audio."
    )
    final_audio_base64 = _concatenate_wav_from_base64_list(base64_audio_parts)
    if not final_audio_base64:
        print(
            "[modules/tts.py] text_to_speech: CRITICAL - Concatenation of audio chunks failed."
        )
